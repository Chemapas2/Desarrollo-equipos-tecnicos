...
def build_radar_comparison(candidates, selected_names):
    ...
    # Paleta más marcada y fácil de distinguir en presentaciones de equipo.
    palette = (
    ...
    for idx, c in enumerate(used):
        ...
        fig.add_trace(
            go.Scatterpolar(
                ...
                fillcolor=color.replace("rgb(", "rgba(").replace(")", ", 0.14)") if color.startswith("rgb(") else None,
                hovertemplate="<b>%{fullData.name}</b><br>%{theta}: %{r:.1f}% vs objetivo<extra></extra>",
            )
        )
    ...
    fig.update_layout(
        polar=dict(
            ...
            angularaxis=dict(
                tickfont=dict(size=13, color="#143b8f"),
                rotation=90,
                direction="clockwise",
        ...
        legend=dict(
            ...
            bordercolor="rgba(20,59,143,0.15)",
            borderwidth=1,
            font=dict(size=12),
        ),
        showlegend=True,
